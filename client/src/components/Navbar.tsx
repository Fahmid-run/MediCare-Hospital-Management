        ...
        <div className="flex items-center justify-between h-16">
          <Link to="/" className="flex items-center gap-2.5 group">
            ...
            <div>
              ...
              <span className="hidden sm:block text-[10px] text-slate-400 -mt-1 leading-none">Hospital Management</span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-2">
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-20 animate-scale-in overflow-hidden">
                    ...
                    <div className="max-h-80 overflow-y-auto scrollbar-thin">
                      {notifications.map((n) => (
                        <div key={n.id} className={cn('px-4 py-3 border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer', !n.read && 'bg-primary-50/40')}>
                          <div className="flex items-start gap-3">
                            <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0', n.read ? 'bg-slate-100 text-slate-400' : 'bg-primary-100 text-primary-600')}>
                              <Icon.Bell size={16} />
                            </div>
                    ...
                    <div className="px-4 py-2.5 text-center">
                      <Link to="/dashboard" className="text-sm font-medium text-primary-600 hover:underline" onClick={() => setNotifOpen(false)}>
            ...
            <Link to="/dashboard" className="hidden lg:flex items-center gap-2 ml-1 pl-3 border-l border-slate-200">
              <Avatar src="https://i.pravatar.cc/300?u=john" name="John Carter" size="sm" ring />
            </Link>

            <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden p-2 rounded-lg text-slate-600 hover:bg-slate-100">
            ...
            <div className="flex flex-col gap-1 pt-2 border-t border-slate-100">
              {navLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn('px-4 py-2.5 rounded-lg text-sm font-medium transition-colors',
